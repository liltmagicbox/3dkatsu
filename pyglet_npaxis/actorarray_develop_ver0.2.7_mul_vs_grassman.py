...
def axis_quat_slerp( self, other, fraction ):
    1
#----------------------------------- quat
...
axis_quat_mul(w,x,y,z, z,y,x,w)
axis_quat_grassman(w,x,y,z, z,y,x,w)
...
#0.024957656860351562 still slow with numba. dont use grassman.
...
def axis_xyz_axisangle(x,y,z): # wehave xyzquat xyzrotmat
    z = heading #sms way
...
def axis_quat_axisangle(w,x,y,z):
    #angle = 2 * acos(qw)
    #diver = sqrt(1-qw*qw)
...
def axis_axisangle_xyz(x,y,z,r):
    3
#--------------------------------------------axis-angle
...
def axis3_front_lookat(front, pos, targetpos, ratio = 1.0):
    ...
    w,x,y,z = axis_axisangle_quat(x,y,z,r)
