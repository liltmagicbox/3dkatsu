...
def just_cos(pos):
    return np.cos(pos)
#just cos slows it 200 to 150, 9000 to 400.wow..
#np: ([100, 1000, 10000, 100000], [65467, 13558, 1646, 154])
#np numba full core: ([100, 1000, 10000, 100000], [18900, 16870, 2991, 407])
...
#@nb.jit#loc=205-<371     cos=240. 160->240..
...
#@nb.jit(nopython=True)#416
#@nb.jit#414
#from numpy import cos 2100 2126 , while np.cos 2071,, but dont this. math.cos not for jit.
@nb.jit(nopython=True , parallel = True)#2000
def just_cos(pos):
    return np.cos(pos)
#np: ([100, 1000, 10000, 100000], [66057, 13883, 1655, 158])
#np numba full core: ([100, 1000, 10000, 100000], [6829, 24722, 14538, 2097]) #as avove,jited.
#np numba full core: ([100, 1000, 10000, 100000], [18737, 16892, 2993, 416]) #when not jited
...
# 205 normal
#ignore# 371 jit
# 9000 jit parallel fullspeed

#loc+cos
# 160 normal
#ignore# 422 cosjit
# 2092 jit parallel fullspeed
