...
# sxcz = sx*cz = 0.5*(sxpz+sxmz)
# cxsz = cx*sz = 0.5*(sxpz-sxmz)
# cxcz = cx*cz = 0.5*(cxpz+cxmz)
# sxsz = sx*sz = -0.5*(cxpz-cxmz)
...
@nb.jit(nopython=True , parallel = True)
def axis_tr_posrotmat(colmat, Tx,Ty,Tz, Rx,Ry,Rz):
    #col major matrix.. for gpu. Rx,Ry,Rz=R,P,Y
    sx = np.sin(Rx)
    cx = np.cos(Rx)
    cy = np.cos(Ry)
    ...
    colmat[0] = cy*cz
    colmat[1] = -cx*sz + sxcz*sy
    colmat[2] = sxsz + cxcz*sy
    colmat[3] =  Tx    
    colmat[4] = cy*sz
    colmat[5] = cxcz + sxsz*sy
    colmat[6] = -sxcz + cxsz*sy
    colmat[7] = Ty     
    colmat[8] = -sy
    colmat[9] = sx*cy
    colmat[10] = cx*cy
...
#-------------------------------------------ver 0.1.1
#pos, rot. no scale/ scale can be vertex *s.
# 0.0718224048614502  single
# 0.015957117080688477 jit

#-----------------------ver012 opt mul
# 73ms-> 69ms at single. nodiff mul.
"""
----- modelmat trs, 1000000
0.0688161849975586 axis_trs_modelmat 1000000
----- modelmat trs, jit 1000000
0.015957355499267578 axis_trs_modelmat 1000000
----- modelmat trs, 1000000
0.06981325149536133 axis_trs_modelmat 1000000
----- modelmat trs, jit 1000000
0.015957117080688477 axis_trs_modelmat 1000000
----- modelmat trs, 1000000
0.0688161849975586 axis_trs_modelmat 1000000
...
# ----- modelmat trs, 1000000
# 0.07579779624938965 axis_trs_modelmat 1000000
# ----- modelmat trs, jit 1000000
# 0.015957117080688477 axis_trs_modelmat 1000000
# (16, 1000000)
# ----- modelmat trs, 1000000
# 0.0723121166229248 axis_trs_modelmat 1000000
# ----- modelmat trs, jit 1000000
# 0.015957117080688477 axis_trs_modelmat 1000000
# ----- modelmat trs, 1000000
# 0.07280540466308594 axis_trs_modelmat 1000000
# ----- modelmat trs, jit 1000000
# 0.01595759391784668 axis_trs_modelmat 1000000


# 10s of sincos. so slow!
# ----- modelmat trs, 1000000
# 0.10771465301513672 axis_trs_modelmat 1000000
# ----- modelmat trs, jit 1000000
# 0.019946813583374023 axis_trs_modelmat 1000000
...
print('----- rotmay xyz',NN)
t=time()
rotmat = axis_xyzmat_rotmat.py_func(dummymat, x,y,z)
#print(rotmat)
...
#print(rotmat.shape)
#print(rotmat[:,0])
#print(rotmat[:,0].dtype)
#<class 'numpy.ndarray'>
#(16, 1000000)
# [ 0.9425213  -0.20407173  0.26459095  0.          0.22317731  0.9737869
#  -0.0439433   0.         -0.2486876   0.10046819  0.96335906  0.
#   0.          0.          0.          1.        ]
# float32

print('----- rotmay xyz  jit',NN)
rotmat = axis_xyzmat_rotmat(dummymat, x,y,z)
t=time()
rotmat = axis_xyzmat_rotmat(dummymat, x,y,z)
#print(rotmat)
#print(rotmat.shape)
