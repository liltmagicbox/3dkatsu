...
def just_cos(pos):
    return np.cos(pos)
#just cos slows it 200 to 150, 9000 to 2000.wow..
#math.cos errors, when use axis methods
...
class Actorarray:
    def __init__(self, modelN):
        # id pos spd acc(location3) (rotation3) vec3*3?
        # attr array, isGravity kinds.
        # shader number.. textureID.. kinds.
        row = 26
        column = modelN
        matrow = 16 # rot trans scale .. for 4x4= matA[i] gpuready.        
        
        #self.physics = np.zeros(row*column).reshape(column,row).astype('float32')#232 vs 170.wow.
        self.array = np.zeros(row*column).reshape(column,row).astype('float32')#232 vs 170.wow.
        self.modelmat = np.zeros(matrow*column).reshape(column,matrow).astype('float32')#gpuready.
        #wow, matarray written once, read once. perfect! all 4x4 is remains as quaternion, fast mul.
        
        self.attribute = np.zeros(matrow*column).reshape(column,matrow).astype('int32') #unit or int?
        #light, collision,,
