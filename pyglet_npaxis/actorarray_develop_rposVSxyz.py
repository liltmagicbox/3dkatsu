...
@nb.jit(nopython=True , parallel = True)
def raxis_xyzmat_rotmat(colmat,rpos):
    #colmat 16*N massive array. access by index
    # x,y,z means Rx,Ry,Rz. thats all.,not! yaw (Z), pitch (Y), roll (X)
    #https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
    x = rpos[0]
...
def xxxaxis_xyzmat_rotmat(m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11,m12,m13,m14,m15,m16, x,y,z):
    ...
    #it makes tuple of array!
    #220build_tuple.93 = build_tuple(items=[Var(m1.1, ...
...
#==================================================
rpos = np.vstack( (x,y,z) )
print(rpos.dtype)
print('----- rotmay rpos',NN)
t=time()
rotmat = raxis_xyzmat_rotmat.py_func(dummymat, rpos)
#print(rotmat)
print(time()-t,'py_func rotmat',NN)
...
print('----- rotmay rpos  jit',NN)
rotmat = raxis_xyzmat_rotmat(dummymat, rpos)
t=time()
rotmat = raxis_xyzmat_rotmat(dummymat, rpos)
#print(rotmat)
#print(rotmat.shape)
...
#cc=raxis_xyzmat_rotmat.inspect_types() #auto. prints.
#print(cc) #you wanna see jit inside?
