...
def axis_vecquat_rotate(x,y,z, qw,qx,qy,qz):
    ...
    tx,ty,tz = axis_vec_cross.py_func( qx*2,qy*2,qz*2, x,y,z )
    #v + qw*tv +qv*tv
    rx = x + qw*tx +qx*tx
    ry = y + qw*ty +qy*ty
    rz = z + qw*tz +qz*tz
...
#for the history dont to this. jit in jit ok, it compiled in all, when type specified.
# @nb.jit(nopython=True , parallel = False)
# def low_axis_quat_front(qw,qx,qy,qz):
#     x=1
#     y=0
#     z=0
#     #tx,ty,tz = axis_vec_cross(  )
#     x1,y1,z1 = qx*2,qy*2,qz*2,
#     x2,y2,z2 = x,y,z
...
w=np.random.rand(100_0000)
x=np.random.rand(100_0000)
y=np.random.rand(100_0000)
z=np.random.rand(100_0000)
axis_vecquat_rotate(x,y,z, w,x,y,z)
axis_vecquat_rotate_qpq(x,y,z, w,x,y,z)
...
y=np.random.rand(100_0000)
z=np.random.rand(100_0000)
front = axis_vecquat_rotate_qpq.py_func(x,y,z, w,x,y,z)
print(time()-t)
...
y=np.random.rand(100_0000)
z=np.random.rand(100_0000)
front = axis_vecquat_rotate(x,y,z, w,x,y,z)
print(time()-t)

exit()
#very little diff. numba
# =======lowest
# 0.9280409812927246
# =======lowest
# 0.8627157211303711
...
#1 axis_vecquat_rotate(says 30%faster)
#2 axis_vecquat_rotate_qpq

#EXTREAMLY FASTER 1core ..no. front had jit.
#0.4727647304534912
#0.06089496612548828

#still EXTREAMLY FASTER , real without jit.
#0.4483613967895508
#0.07878851890563965


#with jit slow, faster
#0.06787967681884766
#0.04489898681640625 
