...
class Actor:
    ...
    def draw(self,gldrawmethod):
        if self.vlist:
            self.vlist.draw(gldrawmethod)
...
def on_draw():
    ...
    #-----------------------NEW CONCEPT.
    # SET PROJECTION, SET VIEW, SET MODEL, DRAW.
    # MAYBE IT'S TIME TO USE GROUP OF PYGLET.
    #DRAW ACTOR, AS IT'S REALLY GOOD WAY. BUT REMEMBER HOW TO RAW DRAW.. --found we cant change it.
    #only get list of vertex_list, not batch, and for v v.draw(GL_POINTS)..
    ...
    glBindTexture(car.texture.target, car.texture.id)
    print(car.texture.target, car.texture.id,'car')
    #car.vlist = vertex_list_indexed1
    car.draw(GL_TRIANGLES)
    #print(vertex_list_indexed1)
    #<pyglet.graphics.vertexdomain.IndexedVertexList object at 0x000001CD7EB22370>
    #isit working?? yeah,well.

    
    glBindTexture(texturetmp.target, texturetmp.id)
    print(texturetmp.target, texturetmp.id)
    #car.draw(GL_TRIANGLES)
