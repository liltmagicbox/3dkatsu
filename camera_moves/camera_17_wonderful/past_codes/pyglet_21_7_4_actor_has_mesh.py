...
void main()
{
    ...
    outcolor = mix(texture(tex0, uvcord), texture(tex1, uvcord), 0.1);
}
"""
...
class Actor:
    def __init__(self):
        ...
        self.mesh_list = [] #[ (vbo,textures),.]
        self.shaderID = shader_default
    ...
    def draw(self,gldrawmode):#deprecated now. use vbo.draw directly.
        #future, ischanged kinds will pass update self.matrix_model kinds..
        if self.vlist:
            self.vlist.draw(gldrawmode)
    def set_uniform_model(self):
        shader_program = self.shaderID
        mmodel= eye4()
        id_modelmat = glGetUniformLocation(shader_program, "Model")#True if row major. [ [1,2,3,4],,
    ...
    def set_uniform_texture(self, i, texture_id):# not use i ,I want change tex.id directly.
        """ texture_id of gpu >>>bind>>> tex(i) of shader.
            actor stores shaderprogram_id.
        """
        if i == 0:
        ...
        shader_program = self.shaderID#WE NEED IT! SINCE EACHMODEL, EACHSHADER MAYBE.
...
class Camera(Actor):
    ...
    def get_matrix_projection(self):#was set_..
        ...
        return mprojection
        #gone to shader.set_view kinds. it's better. yeah..
...
#----------------------since shader contains MVP..
    ...
    #--------MVP    
    projectionmat = world.camera_list[0].get_matrix_projection()
    viewmat = world.camera_list[0].get_matrix_view()
    
    projectionmatID = glGetUniformLocation(shader_default, "Projection")
    glUniformMatrix4fv(projectionmatID,1,True, projectionmat)
    viewmatID = glGetUniformLocation(shader_default, "View")
    ...
    for actor in world.actor_list: # xxx_list named to: 'maybe use for loop'
        actor.set_uniform_model() #for reasonable opengl flow. hopefully not be in draw..
        
        for vbo,textures in actor.mesh_list:#mesh, which contains vbo1-tex1-tex2..
            for i, texture in enumerate(textures):
                actor.set_uniform_texture(i, texture.id)# for direct change..
